...
export default function ResumeUpload() {
  ...
  const { setExtractedData } = useResume();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ detail: 'Upload failed. Please try again.' }));
        throw new Error(errorData.detail || `Server error: ${res.status}`);
      }

      const data = await res.json();
      setExtractedSkills(data.skills || []);
      // Push into global context so other pages can access
      setExtractedData({ skills: data.skills || [], gaps: data.gaps || [], summary: data.summary || '' });
    } catch (err: unknown) {
            ...
            {isUploading ? (
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center">
                ...
                <div className="flex gap-4 w-full max-w-sm">
                  <button onClick={() => { setFile(null); setUploadError(null); }} className="flex-1 px-6 py-3 rounded-xl font-medium text-slate-300 hover:text-white bg-white/5 border border-white/10 hover:bg-white/10 transition-all">Cancel</button>
                  <button onClick={handleUpload} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(37,99,235,0.4)] hover:shadow-[0_0_30px_rgba(37,99,235,0.6)]">Start AI Parser</button>
                </div>
                {uploadError && (
                  <div className="mt-6 w-full max-w-sm bg-red-500/10 border border-red-500/20 text-red-300 text-sm px-4 py-3 rounded-xl">
                    {uploadError}
                  </div>
                )}
              </motion.div>
