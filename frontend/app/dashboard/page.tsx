...
import { Target, Flame, Clock, Award, ArrowRight, BookOpen, Code2, Zap, FileText } from 'lucide-react';
...
export default function DashboardOverview() {
  ...
  const { readinessScore, completedTasks, totalTasks } = useMemo(() => {
    ...
    return { readinessScore: readiness, completedTasks: completed, totalTasks: total };
    ...
    return (
      <div className="max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center">
          <div className="w-24 h-24 bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-white/10 rounded-3xl flex items-center justify-center mb-8">
          ...
          <p className="text-slate-400 text-lg mb-8 max-w-lg">
            Upload your resume to unlock personalized skill gap analysis, learning roadmaps, and AI-powered interview prep.
          </p>
  ...
  // Find the first incomplete task in the first in-progress phase
  const currentPhase = roadmapPhases.find(p => p.status === 'in-progress') || roadmapPhases[0];
  const nextTask = currentPhase?.tasks?.find(t => !t.completed);
          ...
          <div className="h-full bg-[#030712]/60 backdrop-blur-xl border border-white/10 hover:border-white/20 rounded-[2rem] p-8 relative overflow-hidden group transition-all duration-500 shadow-2xl flex flex-col justify-between">
            ...
            <div className="relative z-10 flex items-start justify-between mb-8">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/5 text-slate-300 text-sm font-semibold">
                <Clock className="w-4 h-4 text-blue-400" /> {nextTask?.duration || '—'}
              </div>
            </div>
            
            <div className="relative z-10 mt-auto">
              <span className="text-xs font-black px-3 py-1 uppercase tracking-widest bg-blue-500/20 text-blue-300 rounded-md mb-4 inline-block">
                {nextTask?.type || 'Task'}
              </span>
              <h4 className="text-3xl md:text-4xl font-black text-white mb-4 tracking-tight">
                {nextTask?.title || 'All tasks complete!'}
              </h4>
              <p className="text-slate-400 text-lg mb-8 max-w-xl leading-relaxed">
                {nextTask?.rationale || 'Great work! Navigate to your roadmap to see all your progress.'}
              </p>
            ...
            <div className="relative z-10 w-full flex flex-col items-center">
              ...
              <div className="relative w-48 h-48 mb-8">
                <svg className="w-full h-full transform -rotate-90 drop-shadow-xl" viewBox="0 0 100 100">
                  ...
                  <circle cx="50" cy="50" r="45" fill="none" stroke="url(#score-gradient)" strokeWidth="6" strokeDasharray={`${readinessScore * 2.82} 282`} className="text-purple-500 drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]" strokeLinecap="round" />
                  <defs>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-400">{readinessScore}</span>
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <span className={`text-[10px] font-black px-2.5 py-1 uppercase tracking-widest rounded-md ${
                      skill.priority === 'Critical' ? 'bg-red-500/10 text-red-400' : 'bg-white/5 text-slate-400'
                    }`}>
                      {skill.category}
                    </span>
                  </div>
